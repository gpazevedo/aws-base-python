...
if [ ! -d ".terraform" ]; then
  echo "❌ Error: Bootstrap Terraform not initialized"
  echo "   Please run: make bootstrap-init"
  exit 1
fi

# Read bootstrap outputs
echo "📖 Fetching Terraform state bucket name..."
STATE_BUCKET=$(terraform output -raw terraform_state_bucket 2>/dev/null)
if [ -z "$STATE_BUCKET" ]; then
  echo "❌ Error: Could not read terraform_state_bucket output"
...
echo "📝 Generating backend configuration files..."
...
EOF
  ...
  echo "✅ Created $TERRAFORM_DIR/environments/${ENV}-backend.hcl"
done
